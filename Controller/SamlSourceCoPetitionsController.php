...
class SamlSourceCoPetitionsController extends CoPetitionsController {
  // Class name, used by Cake
  public $name = "SamlSourceCoPetitions";

  public $uses = array("CoPetition",
                       "OrgIdentitySource",
                       "SamlSource.SamlSource",
                       "SamlSource.SamlSourceBackend");
  ...
  protected function execute_plugin_collectIdentifierIdentify($id, $oiscfg, $onFinish, $actorCoPersonId) {
    ...
    // of it. However, we need to pass the $sourceKey to confirm to that
    // backend that we really are trying to create a record.

    // First pull our configuration.
    $args = array();
    $args['conditions']['SamlSource.org_identity_source_id'] = $oiscfg['OrgIdentitySource']['id'];
    $args['contain'] = false;

    $cfg = $this->SamlSource->find('first', $args);
    ...
    $prefix = $cfg['SamlSource']['saml_var_prefix'];
    $sorid_attr = $cfg['SamlSource']['saml_sorid'];
  ...
  protected function execute_plugin_selectOrgIdentityAuthenticate($id, $oiscfg, $onFinish, $actorCoPersonId) {
    ...
    // (since the user will not be registered) but not at the start of account
    ...
    if(!$token) {
      ...
      $redirect = array(
        'plugin'     => 'saml_source',
        'controller' => 'saml_source_co_petitions',
    ...
    if(empty($cfg)) {
      throw new InvalidArgumentException(_txt('er.notfound',
                                              array(_txt('ct.saml_sources.1'),
                                                    $oiscfg['OrgIdentitySource']['id'])));
    }

    if(empty($cfg['SamlSource']['saml_sorid'])) {
      throw new RuntimeException(_txt('er.samlsource.sorid.cfg'));
    }
    ...
    $sorid = getenv($prefix.$sorid_attr);
    if(!$sorid) {
      throw new RuntimeException(_txt('er.samlsource.sorid', array($cfg['SamlSource']['saml_sorid'])));
    }
  ...
  function isAuthorized() {
    ...
    $curToken = $this->parseToken();

    if($petitionId) {
      $enrolleeToken = $this->CoPetition->field('enrollee_token', array('CoPetition.id' => $petitionId));
    }

    // Construct the permission set for this user, which will also be passed to the view.
    $p = array();

    // In the collectIdentifierIdentify step, we require the Enrollee user (so we can collect
    // its data).
    ...
    $p['collectIdentifierIdentify'] = ($enrolleeToken == $curToken);
